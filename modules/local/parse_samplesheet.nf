process PARSE_SAMPLESHEET {
    ...
    output:
    path "parsed_samples.tsv", emit: samples
...
try:
    with open(samplesheet_file, 'r') as f:
        ...
        # Write output file
        with open('parsed_samples.tsv', 'w') as out:
            # Process each sample
            for row in reader:
                ...
                out.write(f"{sample_id}\\t{fastq_r1}\\t{fastq_r2}\\t{chemistry}\\t{json.dumps(meta_map)}\\n")
